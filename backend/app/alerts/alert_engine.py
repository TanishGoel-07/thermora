...
from app.alerts.notifiers import send_sms, send_email, send_push, send_whatsapp
...
def dispatch_alert(db: Session, alert: Alert) -> None:
    ...
    for user in users:
        ...
        if user.phone_number and user.whatsapp_opt_in:
        ...
        for channel in channels:
            ...
            try:
                if channel == AlertChannel.SMS:
                    ...
                    send_whatsapp(user.phone_number, f"{alert.title}: {alert.message}")

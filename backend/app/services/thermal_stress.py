...
def _vulnerability_multiplier(
    ...
    if outdoor_worker:
        multiplier += 0.10
    # Pregnant/postpartum individuals face materially higher heat-related risk
    # (per WHO/NDMA heat-action-plan guidance); modeled via the gender field
    # only as a coarse proxy pending a dedicated pregnancy flag.
    if gender and gender.lower() in ("female", "woman"):
        multiplier += 0.03
    # Prolonged outdoor exposure compounds risk beyond a baseline commute
    if daily_outdoor_exposure_hours and daily_outdoor_exposure_hours > 2:
        multiplier += min((daily_outdoor_exposure_hours - 2) * 0.03, 0.25)
    return multiplier
...
def compute_htsi(
    ...
    vulnerability_multiplier = _vulnerability_multiplier(
        age, has_chronic_condition, outdoor_worker, gender, daily_outdoor_exposure_hours
    )
